...
const SearchData1:React.FC<Props> = ({data,query}) => {
            ...
            <ul>
                {   
                    data && Array.isArray(data) ? (
                        <>
                            {companyListed.length > 0 && <SearchDataLi1 item={companyListed} entity={"Companies"} count={records.compcount} query={query}/>}
                            {companyNonListed.length > 0 && <SearchDataLi1 item={companyNonListed} entity={"Non Listed Companies"} count={records.nonlistcount} query={query}/>}
                            {mf.length > 0 && <SearchDataLi1 item={mf} entity={"Mutual Funds"} count={records.nonlistcount} query={query}/>}
                            {nps.length > 0 && <SearchDataLi1 item={nps} entity={"NPS"} count={records.nonlistcount} query={query}/>}
                            {etf.length > 0 && <SearchDataLi1 item={etf} entity={"ETF"} count={records.nonlistcount} query={query}/>}
                            {crypto.length > 0 && <SearchDataLi1 item={crypto} entity={"CRYPTOCURRENCY"} count={records.coincount} query={query}/>}
                            {forex.length > 0 && <SearchDataLi1 item={forex} entity={"FOREX"} count={records.forexcount} query={query}/>}
                            {commodity.length > 0 && <SearchDataLi1 item={commodity} entity={"COMMODITY"} count={records.cmdtcount} query={query}/>}
                            {index.length > 0 && <SearchDataLi1 item={index} entity={"INDEX"} count={records.inetcount} query={query}/>}
